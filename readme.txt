1) Prepare your input files and put it in the data_dir (later define in config.sh)
...
2) Validate the prepared mapping file
    ...
    source /biodata/dep_psl/grp_rgo/guan/DADA2_pipeline/activate
    validate_mapping_file.py -m prefix_mapping.txt -o ./

3) Edit config file:

    Define the working_dir and data_dir in config file (/biodata/dep_psl/grp_rgo/guan/DADA2_pipeline/config.sh)
