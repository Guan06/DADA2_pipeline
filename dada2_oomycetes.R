...
########################################### for Fungi
path1 <- paste0(path, "/Oom_forward/out")
path2 <- paste0(path, "/Oom_reverse/out")
output_oom <- paste0(output, "Oomycetes/")
...
saveRDS(filtFs, paste0(output_oom, "filtFs.rds"))
saveRDS(filtRs, paste0(output_oom, "filtRs.rds"))

write.table(out, paste0(output_oom, "filter.txt"), quote = F, sep = "\t")
...
saveRDS(errF, paste0(output_oom, "errF.rds"))
saveRDS(errR, paste0(output_oom, "errR.rds"))

p1 <- plotErrors(errF, nominalQ = TRUE)
p2 <- plotErrors(errR, nominalQ = TRUE)
ggsave(paste0(output_oom, "errorF.pdf"), p1)
ggsave(paste0(output_oom, "errorR.pdf"), p2)
...
saveRDS(dadaFs, paste0(output_oom, "dadaFs.rds"))
saveRDS(dadaRs, paste0(output_oom, "dadaRs.rds"))

mergers <- mergePairs(dadaFs, filtFs, dadaRs, filtRs, verbose = TRUE)
saveRDS(mergers, paste0(output_oom, "mergers.rds"))

seqtab <- makeSequenceTable(mergers, orderBy = "abundance")
saveRDS(seqtab, paste0(output_oom, "seqtab.rds"))
